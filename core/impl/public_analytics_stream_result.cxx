  ...
  -> analytics_meta_data
{
  ...
  return analytics_meta_data{
    ...
    core::impl::map_analytics_status(meta.status),
    std::move(warnings),
    metrics,
    std::move(signature),
  };
...
internal_analytics_stream_result::resolve_meta_data(std::error_code ec)
{
  ...
  std::vector<std::promise<std::pair<error, analytics_meta_data>>> to_satisfy;
  {
...
{
  const std::scoped_lock lk{ meta_mutex_ };
  std::promise<std::pair<error, analytics_meta_data>> promise;
...
internal_analytics_stream_result::next(analytics_row_handler&& handler)
{
  ...
  // free). The stream is always owned via shared_ptr (make_shared in dispatch_analytics_stream).
...
void
analytics_stream_result::cancel() const
{
  if (internal_) {
...
auto
analytics_stream_result::iterator::operator*() const -> std::pair<error, analytics_row>
{
  return item_;
}
...
auto
analytics_stream_result::begin() const -> analytics_stream_result::iterator
{
...
auto
analytics_stream_result::end() const -> analytics_stream_result::end_sentinel
{
  return {};
}
} // namespace couchbase
