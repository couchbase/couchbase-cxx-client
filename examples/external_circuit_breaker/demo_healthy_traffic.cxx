  ...
  -> void
{
  ...
  for (int i = 0; i < total_ops; ++i) {
    ...
    // A successful node_id_for() can still yield a falsy node_id (e.g. the
    // key's node is not currently bound to a KV port). Treat that as
    // unresolved rather than passing an empty id to execute(), which would
    // bypass per-node admission/metrics. Matches demo_per_node_isolation.cxx.
    if (nid_err || !target) {
      if (nid_err) {
  ...
  if (failed == 0 && unresolved == 0) {
    ...
    if (unresolved != 0) {
      msg += ", " + std::to_string(unresolved) + " skipped (unresolved node)";
    }
