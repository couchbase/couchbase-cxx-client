...
# Semicolons would otherwise split a name across two list elements.
...
foreach(case_name IN LISTS listing)
  ...
  # The generated file quotes names with [==[ ]==]. A name containing that sequence would close the
  # bracket early and produce a file that is syntactically valid CMake but registers something else
  # -- which is worse than not building. Case names are C++ identifiers today, so this never fires;
  # it is here so that if it ever can, it fires loudly rather than silently.
  # Any "]==" is refused, not just "]==]": a name *ending* in "]==" closes the bracket early too,
  # because what is written is the name followed by "]==]".
  if(case_name MATCHES "\\]==")
    message(FATAL_ERROR "${TEST_EXECUTABLE}: test case name contains ]== and cannot be registered: ${case_name}")
  endif()
