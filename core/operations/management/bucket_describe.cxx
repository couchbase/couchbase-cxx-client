...
bucket_describe_request::make_response(error_context::http&& ctx, const encoded_response_type& encoded) const
{
    ...
    if (const auto* nodes = payload.find("nodesExt"); nodes != nullptr && nodes->is_array()) {
        ...
        for (const auto& node : nodes->get_array()) {
            if (const auto* server_group_name = node.find("serverGroup"); server_group_name != nullptr && server_group_name->is_string()) {
                ...
                if (const auto* services = node.find("services"); services != nullptr && services->is_object()) {
                    server.default_network.kv_plain = services->template optional<std::uint16_t>("kv").value_or(0);
                    server.default_network.kv_tls = services->template optional<std::uint16_t>("kvSSL").value_or(0);
                }
                if (const auto* alt = node.find("alternateAddresses"); alt != nullptr && alt->is_object()) {
                    if (const auto* external = alt->find("external"); external != nullptr && external->is_object()) {
                        if (const auto* hostname = external->find("hostname"); hostname != nullptr && hostname->is_string()) {
                            server.external_network.hostname = hostname->get_string();
                            if (const auto* services = external->find("ports"); services != nullptr && services->is_object()) {
                                server.external_network.kv_plain = services->template optional<std::uint16_t>("kv").value_or(0);
                                server.external_network.kv_tls = services->template optional<std::uint16_t>("kvSSL").value_or(0);
                            }
                        }
                    }
                }
