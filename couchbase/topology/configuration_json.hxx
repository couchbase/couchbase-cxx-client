...
{
template<>
struct traits<couchbase::topology::configuration> {
    template<template<typename...> class Traits>
    static couchbase::topology::configuration as(const tao::json::basic_value<Traits>& v)
    {
        couchbase::topology::configuration result;
        ...
        if (auto* node_locator = v.find("nodeLocator"); node_locator != nullptr && node_locator->is_string()) {
            if (node_locator->get_string() == "ketama") {
                result.node_locator = couchbase::topology::configuration::node_locator_type::ketama;
            } else {
                result.node_locator = couchbase::topology::configuration::node_locator_type::vbucket;
            }
        }
        ...
        if (auto* nodes_ext = v.find("nodesExt"); nodes_ext != nullptr) {
            ...
            for (const auto& j : nodes_ext->get_array()) {
                couchbase::topology::configuration::node n;
                n.index = index++;
                ...
                n.services_tls.eventing = s.template optional<std::uint16_t>("eventingSSL");
                {
                    ...
                    if (alt != o.end()) {
                        for (const auto& entry : alt->second.get_object()) {
                            couchbase::topology::configuration::alternate_address addr;
            ...
            if (result.node_locator == couchbase::topology::configuration::node_locator_type::vbucket) {
