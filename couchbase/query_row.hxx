...
public:
   ...
   *
   * Only call this on a row obtained from a successful @ref query_stream_result#next() (falsy
   * error, engaged optional). The end-of-stream and error sentinels carry an empty @ref query_row,
   * and decoding empty/malformed content throws the serializer's parse exception — gate on the
   * three-state next() contract (check the error, then the optional) before decoding.
   *
