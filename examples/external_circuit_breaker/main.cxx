...
{
  return example::demo::safe_getenv(var).value_or(fallback);
}
...
main() -> int
{
  ...
  if (nid_err) {
    // A failed resolution leaves victim default-constructed (falsy). The
    // failure-injection demos that follow all assume a real node_id;
    // continuing with an empty victim would silently turn each demo into
    // a no-op against the registry's default-constructed key. Bail out
    // here rather than ship a confusing run.
