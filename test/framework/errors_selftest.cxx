...
//
// assert_success replaces every REQUIRE_SUCCESS call site, the most numerous check in the tree, so
// its failure message is what most failures in the migrated suite will look like. These cases pin
// that message. They need the client library for couchbase::error, but no server: every value here
...
assert_error_distinguishes_one_failure_from_another([[maybe_unused]] context& ctx)
{
  ...
  // The couchbase::error overload compares through ec(), which is a second path to the same
  // verdict: nothing above reaches it, so a regression there would surface only in a migrated
  // suite rather than here.
  const couchbase::error wrapped{ expected, "the operation failed" };
  assert_error(wrapped, expected, "an error carrying the expected code passes");

  const auto from_error = message_of([]() {
    assert_error(couchbase::error{ make_error_code(couchbase::errc::common::unambiguous_timeout) },
                 make_error_code(couchbase::errc::key_value::document_not_found));
  });
  assert_contains(from_error, "unambiguous_timeout", "and a mismatch names the code it carried");
  assert_contains(from_error, "document_not_found", "alongside the one that was expected");
}
