...
mock() -> requirement_ptr
{
  return make("the gocaves mock", [](context& ctx) {
    if (!ctx.config().mock) {
      return check_result::missing("the mock is not in use (TEST_USE_GOCAVES is unset)");
    }
    return check_result::ok();
  });
}
...
{
  const std::string name = wanted == server_edition::enterprise ? "enterprise" : "community";
...
storage_backend(std::string backend) -> requirement_ptr
{
  ...
  return make(std::move(description), [backend = std::move(backend)](context& ctx) {
    ...
    if (const auto actual = ctx.storage_backend(); actual != backend) {
