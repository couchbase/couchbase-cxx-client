 ...
 * returns @c subdoc_multi_path_failure (mapped to HTTP 200 / success) when the document exists
 * but one or more of the requested paths are absent.  This happens for every non-transacted
 * document fetched during a transaction, because the transaction XATTRs (@c txn.id, @c txn.atr,
 * etc.) do not exist on those documents.
 *
