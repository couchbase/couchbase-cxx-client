...
{
  {
    ...
    // itself) to avoid deadlocking on the non-recursive mutex.
    std::scoped_lock lock(mutex_);
  ...
  std::function<void(std::optional<couchbase::core::transactions::error_class>)>&& handler)
{
  ...
  switch (hook_.hook_action()) {
    ...
    case protocol::hooks::transactions::HookAction::REMOVE_DOC: {
      // doc location in format: bucket/collection/doc_id, where the collection segment may
      // optionally carry a scope as scope:collection (the scope defaults to _default otherwise)
      std::vector<std::string> doc_locations =
        couchbase::core::utils::split_string(hook_.hook_action_param1(), '/');
      if (doc_locations.size() < 3) {
        throw performer_exception::invalid_argument(
          "hook doc location must be in the format bucket/[scope:]collection/doc_id, got: " +
          hook_.hook_action_param1());
      }
      ...
      if (coll_spec.empty()) {
        throw performer_exception::invalid_argument(
          "hook doc location collection segment must not be empty, got: " +
          hook_.hook_action_param1());
      }
    ...
    case protocol::hooks::transactions::HookAction::BLOCK: {
      int block_ms = 0;
      try {
        block_ms = std::stoi(hook_.hook_action_param1());
      } catch (const std::exception&) {
        throw performer_exception::invalid_argument(
          "hook BLOCK action requires an integer milliseconds parameter, got: " +
          hook_.hook_action_param1());
      }
  ...
  std::optional<const std::string> stage)
{
  ...
  switch (hook_.hook_condition()) {
    ...
    case protocol::hooks::transactions::HookCondition::EQUALS:
      if (!doc_id) {
        throw performer_exception::invalid_argument(
          "hook condition EQUALS requires a doc id, but none was provided");
      }
      return *doc_id == hook_.hook_condition_param2();
    case protocol::hooks::transactions::HookCondition::ON_CALL_AND_EQUALS:
      if (!doc_id) {
        throw performer_exception::invalid_argument(
          "hook condition ON_CALL_AND_EQUALS requires a doc id, but none was provided");
      }
