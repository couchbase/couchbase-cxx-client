...
public:
  ...
  [[nodiscard]] bool has_expired(std::uint32_t safety_margin = 0) const
  {
    ...
    if (timestamp_start_ms_ && expires_after_ms_ && cas_ms > *timestamp_start_ms_) {
      // Compute the effective TTL in std::uint64_t: expires_after_ms + safety_margin would
      // otherwise be evaluated in std::uint32_t and could overflow for large TTLs/margins,
      // flipping the comparison.
      const std::uint64_t expires_after_ms = *expires_after_ms_;
