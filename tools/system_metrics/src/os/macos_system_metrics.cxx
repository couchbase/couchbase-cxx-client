...
auto
MacosSystemMetrics::get_system_boot_time() const -> std::uint64_t
{
  return SystemConstants::instance().boot_time;
}

auto
MacosSystemMetrics::get_logical_cpu_count() const -> std::uint32_t
{
...
auto
MacosSystemMetrics::get_proc_cpu(sm_pid_t pid) const -> proc_cpu_t
{
  ...
  if (sysctl(mib.data(), static_cast<unsigned int>(mib.size()), &pinfo, &len, nullptr, 0) < 0) {
    throw std::system_error(std::error_code(errno, std::system_category()),
                            "MacosSystemMetrics::get_pinfo(): sysctl");
  }
  ...
  if (st != SM_OK) {
    throw std::system_error(std::error_code(st, std::system_category()),
                            "MacosSystemMetrics::get_proc_time: get_proc_times()");
  }
...
auto
MacosSystemMetrics::get_proc_memory(sm_pid_t pid) const -> proc_mem_t
{
  proc_mem_t procmem{};
  proc_taskinfo pti{};
  ...
  if (sz != static_cast<int>(sizeof(pti))) {
    throw std::system_error(
      errno,
      std::system_category(),
      "MacosSystemMetrics::get_proc_memory(): proc_pidinfo(PROC_PIDTASKINFO)");
  }
...
void
MacosSystemMetrics::iterate_proc_threads(sm_pid_t pid, IterateThreadCallback callback)
{
