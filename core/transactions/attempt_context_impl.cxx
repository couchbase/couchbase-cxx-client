...
{
void
wrap_err_callback_for_async_api(std::exception_ptr err, std::function<void(couchbase::error)>&& cb)
{
  if (err) {
    try {
    ...
    ensure_open_bucket(id.bucket(), [this, id, cb = std::move(cb)](std::error_code ec) mutable {
      ...
      do_get(
        ...
        [this, id, cb = std::move(cb)](std::optional<error_class> ec,
                                       std::optional<std::string> err_message,
                                       std::optional<transaction_get_result> res) mutable {
          auto handler = [this, id, err_message, res, cb = std::move(cb)](
                           std::optional<error_class> ec) mutable {
            if (ec) {
              switch (*ec) {
                case FAIL_EXPIRY:
      ...
      [this, cb = std::move(cb), document, content = std::move(content)](
        ...
        try {
          ...
          check_and_handle_blocking_transactions(
            ...
            [this, existing_sm, document, cb = std::move(cb), op_id, content = std::move(content)](
                ...
                [this,
                 ...
                 content =
                  ...
                  if (existing_sm != nullptr &&
                    ...
                    CB_ATTEMPT_CTX_LOG_DEBUG(
                      this, "found existing INSERT of {} while replacing", document);
  ...
  auto error_handler =
    [this](error_class ec, external_exception cause, const std::string& msg, Handler&& cb) {
      ...
      switch (ec) {
        ...
        case FAIL_TRANSIENT:
        case FAIL_AMBIGUOUS:
          return op_completed_with_error(std::forward<Handler>(cb), err.retry());
        case FAIL_HARD:
          return op_completed_with_error(std::forward<Handler>(cb), err.no_rollback());
        default:
    ...
    [this,
     ...
     error_handler = std::move(error_handler)](core::operations::mutate_in_response resp) mutable {
      if (auto ec2 = error_class_from_response(resp); ec2) {
        return error_handler(
          *ec2,
          external_exception_from_response(std::move(resp)),
      ...
      return hooks_.after_staged_replace_complete(
        ...
        [this,
         ...
         resp = std::move(resp)](auto ec) mutable {
          ...
          return op_completed_with_callback(std::forward<Handler>(cb), std::optional(out));
        });
    });
}
      ...
      [this, id, content = std::move(content), cb = std::move(cb)](std::error_code ec) mutable {
        ...
        try {
          ...
          if ((existing_sm != nullptr) && (existing_sm->type() == staged_mutation_type::INSERT ||
            ...
            CB_ATTEMPT_CTX_LOG_DEBUG(
              this, "found existing insert or replace of {} while inserting", id);
            return op_completed_with_error(
              std::move(cb),
                                                            ...
                                                            Handler&& cb)
{
  try {
    ...
    return hooks_.before_check_atr_entry_for_blocking_doc(
      this,
      doc.id().key(),
      [this, delay = std::move(delay), cb = std::forward<Handler>(cb), doc](auto ec) mutable {
        if (ec) {
        ...
        return active_transaction_record::get_atr(
          cluster_ref(),
          atr_id,
          [this, delay = std::move(delay), cb = std::move(cb), doc](
...
attempt_context_impl::remove(const transaction_get_result& document, VoidCallback&& cb)
{
  ...
  return cache_error_async(cb, [&]() mutable {
    check_if_done(cb);
    ensure_open_bucket(
      document.bucket(), [this, document, cb = std::move(cb)](std::error_code ec) mutable {
        if (ec) {
          return op_completed_with_error(std::move(cb),
                                         transaction_operation_failed(FAIL_OTHER, ec.message()));
        }
        staged_mutation* existing_sm = staged_mutations_->find_any(document.id());
        auto error_handler = [this](error_class ec, std::string msg, VoidCallback&& cb) mutable {
        ...
        return check_and_handle_blocking_transactions(
          ...
          [this,
           ...
           op_id,
           error_handler =
             std::move(error_handler)](std::optional<transaction_operation_failed> err1) mutable {
            if (err1) {
  ...
  -> std::exception_ptr
{
  ...
  if (!resp.meta.errors) {
    ...
    external_exception cause = (resp.ctx.ec == couchbase::errc::common::service_not_available
                               ...
                               QueryCallback&& cb)
{
  ...
  wrap_query(statement,
             ...
             [this, cb = std::move(cb)](std::exception_ptr err,
                                        core::operations::query_response resp) mutable {
               if (err) {
                 return op_completed_with_error(std::move(cb), std::move(err));
               }
  ...
  std::function<void(std::exception_ptr, core::operations::query_response)>&& cb)
{
  ...
  return hooks_.before_query(
    this, statement, [this, statement, req, cb = std::move(cb)](auto ec) mutable {
      if (ec) {
        auto err = std::make_exception_ptr(
      ...
      return overall_.cluster_ref().execute(
        req, [this, req, cb = std::move(cb)](core::operations::query_response resp) mutable {
          ...
          return hooks_.after_query(
            this,
            resp.ctx.statement,
            [this, resp = std::move(resp), cb = std::move(cb)](auto ec) mutable {
              if (ec) {
                auto err = std::make_exception_ptr(
    ...
    op_list_.set_query_mode(
      [this, statement, options, query_context, cb]() mutable {
        ...
        query_begin_work(query_context,
                         [this, statement, query_context, options, cb = std::move(cb)](
  ...
  std::optional<std::string> query_context) -> core::operations::query_response
{
  ...
  query(statement,
        options,
        query_context,
        [barrier](std::exception_ptr err, std::optional<core::operations::query_response> resp) {
          if (err) {
            return barrier->set_exception(std::move(err));
          }
  ...
  std::function<void(std::exception_ptr, std::optional<transaction_get_result>)>&& cb)
{
  cache_error_async(cb, [&]() {
    ...
    return wrap_query(
      ...
      true,
      {},
      [this, id, optional, cb = std::move(cb)](std::exception_ptr err,
                                               core::operations::query_response resp) mutable {
  ...
  std::function<void(std::exception_ptr, std::optional<transaction_get_result>)>&& cb)
{
  cache_error_async(cb, [&]() {
    ...
    return wrap_query(
      ...
      true,
      {},
      [this, id, cb = std::move(cb)](std::exception_ptr err,
                                     core::operations::query_response resp) mutable {
        if (err) {
          try {
  ...
  std::function<void(std::exception_ptr, std::optional<transaction_get_result>)>&& cb)
{
  cache_error_async(cb, [&]() {
    ...
    return wrap_query(
      ...
      true,
      {},
      [this, id = document.id(), cb = std::move(cb)](
  ...
  retry_op<void>([this, &ambiguity_resolution_mode]() {
    try {
      ...
      overall_.cluster_ref().execute(req, [barrier](core::operations::mutate_in_response resp) {
        barrier->set_value(result::create_from_subdoc_response(std::move(resp)));
      });
                                              ...
                                              std::optional<const std::string> doc_id) -> bool
{
  ...
  bool hook = hooks_.has_expired_client_side(this, place, std::move(doc_id));
  if (over) {
  ...
  try {
    if (staged_mutations_->empty()) {
         ...
         &lock](error_class ec,
          ...
          switch (ec) {
            ...
            case FAIL_AMBIGUOUS:
              ...
              return overall_.after_delay(
                std::chrono::milliseconds(1), [this, doc_id, &lock, fn = std::move(fn)]() mutable {
      ...
      return hooks_.before_atr_pending(
        this,
        [this, id, prefix, fn = std::move(fn), error_handler = std::move(error_handler)](
          auto ec) mutable {
          if (ec) {
          ...
          return overall_.cluster_ref().execute(
            req,
            [this, fn = std::move(fn), error_handler = std::move(error_handler)](
              core::operations::mutate_in_response resp) mutable {
              auto ec = error_class_from_response(resp);
              if (ec) {
                return error_handler(
  ...
  try {
    ...
    return hooks_.before_doc_get(
      ...
      [this,
       ...
       cb = std::move(cb)](auto ec) mutable {
        if (ec) {
                                                 ...
                                                 std::optional<transaction_get_result>)>&& cb)
{
  ...
  try {
    overall_.cluster_ref().execute(
      req, [this, id, cb = std::move(cb)](core::operations::lookup_in_response resp) {
        auto ec = error_class_from_response(resp);
        if (ec) {
              ...
              if (!ec3) {
                if (doc) {
                  ...
                  if (!doc->links().is_document_in_transaction()) {
                    ...
                    CB_ATTEMPT_CTX_LOG_TRACE(
                      this, "doc {} not in txn - was inserted outside txn", id);
                    return op_completed_with_error(
                      std::forward<Handler>(cb),
...
{
  remove(transaction_get_result(doc), [handler = std::move(handler)](std::exception_ptr e) mutable {
    wrap_err_callback_for_async_api(std::move(e), std::move(handler));
  });
}
...
void
attempt_context_impl::handle_err_from_callback(std::exception_ptr e)
{
  try {
