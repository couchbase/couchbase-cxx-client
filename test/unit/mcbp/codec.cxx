...
get_random_key_carries_its_collection_id_in_extras([[maybe_unused]] context& ctx)
{
  ...
  // Every read below indexes the frame directly, so its extent is asserted first: a regression that
  // returned a successful but truncated frame would otherwise read past the buffer instead of
  // failing an assertion.
  constexpr std::size_t header_size = 24;
  assert_true(wire.size() >= header_size + sizeof(std::uint32_t),
              "the encoded frame carries the header and the extras");
  ...
  // The extras body follows the 24-byte header. Reading the id back out of it is what holds the
  // value: a length of four is equally true of four zero bytes and of a reversed id.
  auto extras_id = (static_cast<std::uint32_t>(wire[header_size]) << 24) |
                   (static_cast<std::uint32_t>(wire[header_size + 1]) << 16) |
                   (static_cast<std::uint32_t>(wire[header_size + 2]) << 8) |
                   static_cast<std::uint32_t>(wire[header_size + 3]);
  assert_eq(extras_id,
            std::uint32_t{ 0x0000'00ff },
            "the collection id is written into the extras in network order");
}
...
a_get_request_survives_an_encode_decode_round_trip([[maybe_unused]] context& ctx)
{
  ...
  orig.vbucket_ = 7;
  // Every byte distinct, so a field read at the wrong offset or with the wrong width shows up as
  // a different value instead of matching by luck. A byte-order change applied to both sides is
  // out of reach here by construction -- the round trip reproduces the input whatever order the
  // matching helpers agree on -- and is held instead by the byteswap unit suite, which drives the
  // conversions for both host orders.
