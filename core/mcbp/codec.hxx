...
public:
  ...
  [[nodiscard]] auto encode_packet(const packet& packet) const
    -> tl::expected<std::vector<std::byte>, std::error_code>;
  [[nodiscard]] auto decode_packet(gsl::span<std::byte> input) const
    -> std::tuple<packet, std::size_t, std::error_code>;
  [[nodiscard]] auto decode_packet(gsl::span<std::byte> header, gsl::span<std::byte> body) const
    -> std::tuple<packet, std::size_t, std::error_code>;
