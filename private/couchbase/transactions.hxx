...
class transactions
{
  public:
     ...
     */
    transactions(std::shared_ptr<core::cluster> cluster, const transaction_config& config);
     ...
     */
    transaction_result run(logic&& code);

    transaction_result run(const per_transaction_config& config, logic&& code);
    ...
    void run(const per_transaction_config& config, async_logic&& code, txn_complete_callback&& cb);
     ...
     */
    [[nodiscard]] transaction_config& config()
    {
        return config_;
    }
