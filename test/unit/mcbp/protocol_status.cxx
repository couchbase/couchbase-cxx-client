...
// map() hands map_status_code a value derived from the same enum its switch reads, so nothing above
// would notice if an enumerator drifted from the protocol value it transcribes: the case would send
// the drifted value and match the drifted branch, while the real code fell through. These pin every
// status this suite maps to the value in kv_engine/include/mcbp/protocol/status.h, at compile time,
// so the drift cannot be built rather than being reported by whichever case happened to cover it.
static_assert(static_cast<std::uint16_t>(key_value_status_code::durability_impossible) == 0xa1);
static_assert(static_cast<std::uint16_t>(key_value_status_code::durability_invalid_level) == 0xa0);
static_assert(static_cast<std::uint16_t>(key_value_status_code::exists) == 0x02);
static_assert(static_cast<std::uint16_t>(key_value_status_code::invalid) == 0x04);
static_assert(static_cast<std::uint16_t>(key_value_status_code::locked) == 0x09);
static_assert(static_cast<std::uint16_t>(key_value_status_code::not_found) == 0x01);
static_assert(static_cast<std::uint16_t>(key_value_status_code::not_locked) == 0x0e);
static_assert(static_cast<std::uint16_t>(key_value_status_code::not_stored) == 0x05);
static_assert(static_cast<std::uint16_t>(key_value_status_code::range_scan_complete) == 0xa7);
static_assert(static_cast<std::uint16_t>(key_value_status_code::range_scan_more) == 0xa6);
static_assert(static_cast<std::uint16_t>(key_value_status_code::rate_limited_max_commands) == 0x33);
static_assert(static_cast<std::uint16_t>(key_value_status_code::rate_limited_max_connections) ==
              0x32);
static_assert(static_cast<std::uint16_t>(key_value_status_code::rate_limited_network_egress) ==
              0x31);
static_assert(static_cast<std::uint16_t>(key_value_status_code::rate_limited_network_ingress) ==
              0x30);
static_assert(static_cast<std::uint16_t>(key_value_status_code::scope_size_limit_exceeded) == 0x34);
static_assert(static_cast<std::uint16_t>(
                key_value_status_code::subdoc_can_only_revive_deleted_documents) == 0xd6);
static_assert(static_cast<std::uint16_t>(
                key_value_status_code::subdoc_deleted_document_cannot_have_value) == 0xd7);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_delta_invalid) == 0xc8);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_doc_not_json) == 0xc6);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_doc_too_deep) == 0xc4);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_invalid_combo) == 0xcb);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_invalid_xattr_order) ==
              0xd4);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_multi_path_failure) == 0xcc);
static_assert(
  static_cast<std::uint16_t>(key_value_status_code::subdoc_multi_path_failure_deleted) == 0xd3);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_num_range_error) == 0xc7);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_path_exists) == 0xc9);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_path_invalid) == 0xc2);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_path_mismatch) == 0xc1);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_path_not_found) == 0xc0);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_path_too_big) == 0xc3);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_success_deleted) == 0xcd);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_value_cannot_insert) ==
              0xc5);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_value_too_deep) == 0xca);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_xattr_cannot_modify_vattr) ==
              0xd2);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_xattr_invalid_flag_combo) ==
              0xce);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_xattr_invalid_key_combo) ==
              0xcf);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_xattr_unknown_macro) ==
              0xd0);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_xattr_unknown_vattr) ==
              0xd1);
static_assert(static_cast<std::uint16_t>(key_value_status_code::subdoc_xattr_unknown_vattr_macro) ==
              0xd5);
static_assert(static_cast<std::uint16_t>(key_value_status_code::success) == 0x00);
static_assert(static_cast<std::uint16_t>(key_value_status_code::sync_write_ambiguous) == 0xa3);
static_assert(static_cast<std::uint16_t>(key_value_status_code::sync_write_in_progress) == 0xa2);
static_assert(static_cast<std::uint16_t>(key_value_status_code::sync_write_re_commit_in_progress) ==
              0xa4);
static_assert(static_cast<std::uint16_t>(key_value_status_code::too_big) == 0x03);
static_assert(static_cast<std::uint16_t>(key_value_status_code::xattr_invalid) == 0x87);

// The opcode argument carries the same hole. map_status_code branches on insert and unlock alone,
// and map() passes those same enumerators, so a drift would move the case and the branch together
// while a real 0x02 or 0x95 response took the other path. The other opcodes these cases pass stand
// only for "not insert" and "not unlock", so their values do not matter here.
static_assert(static_cast<std::uint8_t>(client_opcode::insert) == 0x02);
static_assert(static_cast<std::uint8_t>(client_opcode::unlock) == 0x95);
