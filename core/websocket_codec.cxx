...
signature_is_valid(const std::string& session_key, const std::string& signature) -> bool
{
  ...
  std::string salted_key{ fmt::format("{}{}", session_key, websocket_guid) };
          ...
          case opcode_continuation:
            if (frame.expected_continuation) {
              std::copy(frame.payload.begin(),
                        frame.payload.end(),
                        std::back_insert_iterator(partial_response_->payload));
            } else {
              if (partial_response_->type == opcode_text) {
                ctx.callbacks.on_text(ctx.ws, partial_response_->payload);
              } else {
                ctx.callbacks.on_binary(ctx.ws, partial_response_->payload);
              }
            }
            break;
          default:
...
{
  if (length <= 125) { // 7 bit
    std::uint8_t field_7_bit = static_cast<std::uint8_t>(length) | flag_mask;
...
decode_frame(gsl::span<std::byte> data, bool expected_continuation) -> decode_status
{
  ...
  bool expect_more = (first_byte & flag_fin) == 0;
  std::uint8_t frame_type = first_byte & opcode_mask;
  ...
  auto second_byte = static_cast<std::uint8_t>(data[1]);

  bool masked = (second_byte & flag_mask) != 0;

  std::uint8_t length_7 = second_byte & payload_length_7_mask;
