...
TEST_CASE("unit: transformer to deduplicate JSON keys", "[unit]")
{
  ...
  CHECK_THROWS_WITH(tao::json::from_string(input),
                    ContainsSubstring("duplicate JSON object key \"answer\""));
...
TEST_CASE("unit: join strings", "[unit]")
{
  ...
  REQUIRE(couchbase::core::utils::join_strings(field_specs, ",") ==
          "testkey:string,volume:double,id:integer");
}
...
TEST_CASE("unit: join strings (fmt version)", "[unit]")
{
  ...
  field_specs.emplace_back("volume:double");
  field_specs.emplace_back("id:integer");
  REQUIRE(couchbase::core::utils::join_strings_fmt(field_specs, ",") ==
          "testkey:string,volume:double,id:integer");
}
...
{
  std::string os_version = fmt::format(";{}/{};{}/0x{:x}",
...
{
  return fmt::format(input, fmt::arg("build", COUCHBASE_CXX_CLIENT_VERSION_BUILD));
}

TEST_CASE("unit: semantic version string", "[unit]")
{
  REQUIRE(couchbase::core::meta::parse_git_describe_output("1.0.0-beta.4-16-gfbc9922") ==
          with_build("1.0.0-beta.4+16.{build}.fbc9922"));
  REQUIRE(couchbase::core::meta::parse_git_describe_output("1.0.0-16-gfbc9922") ==
