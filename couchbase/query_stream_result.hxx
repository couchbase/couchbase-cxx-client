...
public:
   ...
   * @volatile
   */
  [[nodiscard]] auto next() const -> std::future<std::pair<error, std::optional<query_row>>>;
   ...
   * @volatile
   */
  [[nodiscard]] auto meta_data() const -> std::future<std::pair<error, query_meta_data>>;
   ...
   *
   * Dereferencing yields `std::pair<error, query_row>`: for a data row the error is falsy; if the
   * stream ends with an error, the iterator visits exactly one final element whose error is truthy
