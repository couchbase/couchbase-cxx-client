...
public:
   ...
   *    the requested index is at or beyond the bucket's configured replica count, or that count is
   *    zero. Reported without a network round trip.
   *
   * @exception errc::key_value::replica_index_currently_unavailable
   *    the index exists for this bucket, but the copy cannot be read now: the vbucket map's row is
   *    shorter than the configured count, or it does not place that replica on a node the topology
   *    lists.
   *
   * @exception errc::common::unambiguous_timeout
   *
