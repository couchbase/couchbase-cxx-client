...
struct traits<couchbase::management::search::index> {
    ...
    static couchbase::management::search::index as(const tao::json::basic_value<Traits>& v)
    {
        ...
        if (const auto* params = v.find("params"); params != nullptr && params->is_object()) {
            result.params_json = couchbase::utils::json::generate(*params);
        }
        ...
        if (const auto* params = v.find("sourceParams"); params != nullptr && params->is_object()) {
            result.source_params_json = couchbase::utils::json::generate(*params);
        }
        if (const auto* params = v.find("planParams"); params != nullptr && params->is_object()) {
            result.plan_params_json = couchbase::utils::json::generate(*params);
        }
        return result;
    }
};
