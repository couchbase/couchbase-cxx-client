...
{
  const std::scoped_lock lock{ mutex_ };
  // extract() returns an empty node handle (it does not throw) when the key is absent, so guard
  // against it explicitly; calling mapped() on an empty node handle is undefined behaviour.
  auto node = spans_.extract(id);
  if (!node) {
    throw performer_exception::not_found(fmt::format("Span with id {} not found", id));
  }
  return node.mapped();
