  ...
  , public operation_map
{
  ...
  public:
    ...
    void write_and_subscribe(std::shared_ptr<mcbp::queue_request> request, std::shared_ptr<response_handler> handler)
    {
        ...
        auto [data, ec] = codec_.encode_packet(*request);
        if (ec) {
            LOG_DEBUG("unable to encode packet. opaque={}, ec={}", opaque, ec.message());
            request->try_callback({}, ec);
            return;
        }

        if (stopped_) {
            LOG_WARNING("cancel operation while trying to write to closed mcbp session, opaque={}", opaque);
