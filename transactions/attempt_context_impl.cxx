        ...
        try {
            ...
            check_and_handle_blocking_transactions(
              ...
              [this, existing_sm = std::move(existing_sm), document = std::move(document), cb = std::move(cb), content](
                std::optional<transaction_operation_failed> e1) {
                  if (e1) {
                      return op_completed_with_error(cb, *e1);
                  }
                  select_atr_if_needed_unlocked(
                    document.id(),
                    [this, existing_sm = std::move(existing_sm), document = std::move(document), cb = std::move(cb), content](
                      std::optional<transaction_operation_failed> e2) {
                        if (e2) {
...
attempt_context_impl::remove(const transaction_get_result& document, VoidCallback&& cb)
{
    ...
    return cache_error_async(std::move(cb), [&]() {
        ...
        check_and_handle_blocking_transactions(
          ...
          [this, document = std::move(document), cb = std::move(cb), error_handler](std::optional<transaction_operation_failed> err1) {
              if (err1) {
                  return op_completed_with_error(cb, *err1);
              }
              select_atr_if_needed_unlocked(
                document.id(),
                [document = std::move(document), cb = std::move(cb), this, error_handler](
                  std::optional<transaction_operation_failed> err2) {
                    if (err2) {
                        return op_completed_with_error(cb, *err2);
                    }
...
void
attempt_context_impl::query(const std::string& statement, const transaction_query_options& options, QueryCallback&& cb)
{
    return cache_error_async(std::move(cb), [&]() {
        check_if_done(cb);
...
attempt_context_impl::query(const std::string& statement, const transaction_query_options& options)
{
...
attempt_context_impl::set_atr_pending_locked(const core::document_id& id, std::unique_lock<std::mutex>&& lock, Handler&& fn)
{
    try {
        if (staged_mutations_->empty()) {
            ...
            auto error_handler = [this, &lock, fn](error_class ec, const std::string& message, const core::document_id& doc_id) {
                ...
                switch (ec) {
                    ...
                    case FAIL_AMBIGUOUS:
                        ...
                        return set_atr_pending_locked(doc_id, std::move(lock), fn);
                    case FAIL_TRANSIENT:
                        // Retry txn
                        return fn(err.retry());
                    case FAIL_HARD:
                    ...
                    if (!ec) {
                        if (doc->links().is_document_in_transaction()) {
                            ...
                            active_transaction_record::get_atr(
                              cluster_ref(),
                              doc_atr_id,
                              [this, id, doc, cb = std::move(cb)](std::error_code ec2, std::optional<active_transaction_record> atr) {
                                  if (!ec2 && atr) {
                                                         ...
                                                         const std::string& message)
{
    ...
    switch (ec) {
        ...
        case FAIL_CAS_MISMATCH: {
            ...
            auto error_handler = [this, id, content, cb](error_class ec2, std::string err_message) {
            ...
            return get_doc(
              ...
              [this, id, content, cb, error_handler, delay](
                std::optional<error_class> ec3, std::optional<std::string> err_message, std::optional<transaction_get_result> doc) {
                  if (!ec3) {
                      if (doc) {
                      ...
                      return error_handler(*ec3, *err_message);
                  }
              });
            break;
        }
