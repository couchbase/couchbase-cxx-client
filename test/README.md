...
A dropped `std::error_code` is precisely the class of defect this suite exists to catch, and a test that drops one is asserting nothing while occupying a line in the report. The compiler will not stop you: whatever `[[nodiscard]]` the operation carries is satisfied by calling `get()` on the future it returned, and `std::future::get()` carries none of its own, so the example above compiles silently. The rule is yours to keep.
...
The runner prints, at the end of every run, the cases a **requirement** turned away, grouped by what was missing. A `skip()` called from a case body — or from a requirement's own `check()`, where the phase decides before any requirement has answered — is counted in the totals but not grouped here, because there is no requirement to group it under:
...
**No dependence on case order.** Each ctest entry runs in its own process; run the binary directly and its cases share one, along with the context's probe answers and connection — see [Sharing](#sharing). Nothing a case body creates outlives it either way. A case that only passes after its neighbour has run is a case that will fail alone, under `-R`, which is how anybody debugs it.
...
Every ctest entry is its own process, so under `ctest` a case is isolated from its neighbours by construction: nothing survives from one to the next.

Running a binary directly is the other case. Its cases share the process, and with it the context's answers to the requirement probes and the cluster connection behind them — deliberately, because re-opening a connection and re-asking the same questions once per case is a denial-of-service against the cluster the suite is testing. That is the whole of what is shared, it belongs to the **context** rather than to any case, and no case declares it.
...
| `TEST_BUCKET`, `OTHER_TEST_BUCKET` | `config().bucket`, `config().other_bucket` — `TEST_OTHER_BUCKET` is read after it, for environments that already set that spelling |
