...
transactions::transactions(std::shared_ptr<core::cluster> cluster, const transaction_config& config)
  : cluster_(cluster)
  , config_(config)
...
transaction_result
wrap_run(transactions& txns, const per_transaction_config& config, std::size_t max_attempts, Handler&& fn)
{
    ...
    while (attempts++ < max_attempts) {
        ...
        auto f = barrier->get_future();
        auto finalize_handler = [&, barrier](std::optional<transaction_exception> err, std::optional<transaction_result> result) {
            if (result) {
...
transaction_result
transactions::run(const per_transaction_config& config, logic&& code)
{
    return wrap_run(*this, config, max_attempts_, std::move(code));
}
