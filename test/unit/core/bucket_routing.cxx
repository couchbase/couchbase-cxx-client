...
// map_and_send() parks a command on the deferred queue after asking for a session, and only a
// bootstrap completion drains that queue. A true return from connect_session() therefore promises
// that a parked command will be woken -- by a pending bootstrap, or by the cancellation a closed
// bucket applies.
...
void
a_closed_bucket_reports_success_because_parking_cancels_instead([[maybe_unused]] context& ctx)
{
  ...
  fixture.install_config(3, /* rev = */ 1, /* with_key_value_port = */ true);
  fixture.get()->close();
  assert_true(fixture.get()->connect_session(0), "parking is what completes the command");
}

void
a_command_is_not_parked_behind_a_session_that_cannot_start([[maybe_unused]] context& ctx)
{
  bucket_fixture fixture{};
  ...
  assert_true(completed, "the request was completed rather than parked");
...
tests() -> test_suite
{
  return {
    suite_name,
    {
      ...
      { CASE(a_closed_bucket_reports_success_because_parking_cancels_instead), {}, timeout::fast },
      { CASE(a_command_is_not_parked_behind_a_session_that_cannot_start), {}, timeout::fast },
