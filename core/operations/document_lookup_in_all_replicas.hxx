              ...
              core->with_bucket_configuration(
                ...
                [core, id, timeout, specs, parent_span, read_preference, h = std::forward<Handler>(h)](
                  std::error_code ec, const topology::configuration& config) mutable {
                    ...
                    if (ec) {
                        return h(response_type{ make_subdocument_error_context(make_key_value_error_context(ec, id), ec, {}, {}, false) });
                    }
                    ...
                    for (const auto& node : nodes) {
                        if (node.is_replica) {
                            ...
                            core->execute(lookup_in_request{ document_id{ id }, {}, {}, false, specs, timeout }, [ctx](auto&& resp) {
                                handler_type local_handler{};
                                {
                                    std::scoped_lock lock(ctx->mutex_);
                                    if (ctx->done_) {
