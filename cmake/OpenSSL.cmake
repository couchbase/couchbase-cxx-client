...
option(COUCHBASE_CXX_CLIENT_USE_BORINGSSL_PREFIX "Use BoringSSL option to prefix symbols" TRUE)
...
elseif(COUCHBASE_CXX_CLIENT_STATIC_BORINGSSL)
  # 2023-09-29
  SET(COUCHBASE_CXX_CLIENT_BORINGSSL_DEFAULT_SHA "bd20800c22fc8402611b537287bd6948c3f2a5a8")
  if(NOT DEFINED COUCHBASE_CXX_CLIENT_BORINGSSL_SHA)
    message(STATUS "COUCHBASE_CXX_CLIENT_BORINGSSL_SHA not set, using default SHA.")
    set(COUCHBASE_CXX_CLIENT_BORINGSSL_SHA "${COUCHBASE_CXX_CLIENT_BORINGSSL_DEFAULT_SHA}")
  endif()
  message(STATUS "COUCHBASE_CXX_CLIENT_BORINGSSL_SHA=${COUCHBASE_CXX_CLIENT_BORINGSSL_SHA}")
  ...
  if(NOT BORINGSSL_SETUP)
    ...
    message("Cloning BoringSSL...")
    FetchContent_Populate(boringssl)
    message("Cloned BoringSSL to ${boringssl_SOURCE_DIR}")
    ...
    if(WIN32)
      execute_process(
        COMMAND
          ...
          ${CMAKE_BUILD_TYPE} ${COUCHBASE_CXX_CLIENT_BORINGSSL_PIC} ${COUCHBASE_CXX_CLIENT_BORINGSSL_PREFIX}
      ...
      execute_process(
        COMMAND
          ...
          ${CMAKE_BUILD_TYPE} ${COUCHBASE_CXX_CLIENT_BORINGSSL_PIC} ${COUCHBASE_CXX_CLIENT_BORINGSSL_PREFIX}
          "-DCMAKE_C_COMPILER=${CMAKE_C_COMPILER} -DCMAKE_CXX_COMPILER=${CMAKE_CXX_COMPILER} -DCMAKE_AR=${CMAKE_AR}"
        WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}/bin
        RESULT_VARIABLE BUILD_RESULT)
    endif()
    if(NOT
  ...
  file(READ "${BORINGSSL_OUTPUT_DIR}/boringssl_sha.txt" BORINGSSL_SHA)
  string(STRIP "${BORINGSSL_SHA}" BORINGSSL_SHA)
  if(NOT COUCHBASE_CXX_CLIENT_BORINGSSL_SHA STREQUAL BORINGSSL_SHA)
    message(FATAL_ERROR "Failed to build BoringSSL with expected SHA.  Used=${BORINGSSL_SHA}, expected=${COUCHBASE_CXX_CLIENT_BORINGSSL_SHA}.")
  endif()
  message(STATUS "COUCHBASE_CXX_CLIENT_BORINGSSL_SHA=${COUCHBASE_CXX_CLIENT_BORINGSSL_SHA}")
