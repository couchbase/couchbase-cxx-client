...
TEST_CASE("integration: get replica by index", "[integration]")
{
  ...
  SECTION("the second replica returns the document")
  {
    ...
    auto [first_err, first_result] =
      collection
        .get_replica(key,
                     couchbase::get_replica_strategy::from_index(couchbase::replica_index::first))
        .get();
    REQUIRE_SUCCESS(first_err.ec());
    ...
    // Every copy holds the same payload and is_replica() is a constant, so the
    // serving node is the only thing that tells one requested index from
    // another. A route that ignored the index would answer both reads from one
    // node and leave the rest of this section passing.
    REQUIRE(static_cast<bool>(first_result.node_id()));
    REQUIRE(static_cast<bool>(result.node_id()));
    REQUIRE(result.node_id() != first_result.node_id());
  }
