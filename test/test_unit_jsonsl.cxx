...
struct parser_event {
  ...
  {
    os << "{type: " << event.type_ << ", value: " << event.value_
...
TEST_CASE("unit: jsonsl parse with limited depth and JSON pointer", "[unit]")
{
  ...
  // depth = 3
  {
    ...
    REQUIRE(state.events[6] == parser_event(parser_event_type::finish_array,
                                            R"([42,"43",44,[3.14,null,false],true])"));
