...
//
// io::mcbp_session invokes it from three places: the normal response path, the close path where
// every in-flight operation is cancelled, and a write attempted after the session has stopped. The
// session snapshot is present in all three; what distinguishes the two cancellation paths from the
// first is the error code. io::mcbp_session needs an io_context and a live connection to construct,
...
{
  // std::nullopt is a synthetic input here, not a description of the production path: every call
  // site in io::mcbp_session supplies a session snapshot, the close path included. Constructing one
  // needs an io_context and a live connection, so the cases pass nullopt and pin the interface --
  // that a handler still receives the error and retry reason when no connection metadata comes with
  // them.
  captured_call out;
  recording_handler handler{ out };
