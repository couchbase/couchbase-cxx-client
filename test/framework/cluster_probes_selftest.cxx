...
// This file builds its messages with fmt, so it includes fmt itself rather than leaning on
// whichever framework header happens to carry it.
...
tests() -> test_suite
{
  return {
    suite_name,
    {
      ...
      // body asserts on, so a probe wrongly answering false would skip this case instead of
      // failing it -- and the one case whose job is to catch that would be the one that never
      // runs.
      { CASE(the_kv_service_is_always_present), { needs::real_cluster() }, timeout::integration },
      { CASE(the_topology_probes_answer_with_counts),
        { needs::real_cluster(), needs::service("kv") },
        timeout::integration },
      { CASE(the_storage_backend_is_one_the_server_can_name),
