    ...
    std::vector<topology::configuration::node> candidate_nodes{};
    {
      ...
      std::copy_if(config_.nodes.begin(),
                   ...
                   [this, type, &undesired_node](const topology::configuration::node& node) {
                     return node.endpoint(options_.network, type, options_.enable_tls) !=
                            undesired_node;
                   });
    }

    if (candidate_nodes.empty()) {
      // Could not find any other nodes, use the undesired one
      return lookup_node(type, undesired_node);
    }
