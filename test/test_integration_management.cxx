    ...
    SECTION("user and groups crud")
    {
...

        {
            ...
            test::utils::wait_until([&integration, &user, &resp]() {
                ...
                req.user = user;
                resp = test::utils::execute(integration.cluster, req);
                return std::all_of(resp.errors.begin(), resp.errors.end(), [](const auto& message) {
                    return message.find("Groups do not exist") == std::string::npos;
                });
            });
            REQUIRE_FALSE(resp.ctx.ec);
        }
...
TEST_CASE("integration: query index management", "[integration]")
{
    ...
    SECTION("primary index")
    {
        ...
        // TODO: Drop bucket when MB-50725 is fixed
    }
    ...
    SECTION("non primary index")
    {
...

        {
            ...
            REQUIRE_FALSE(resp.ctx.ec);
            auto index =
              std::find_if(resp.indexes.begin(), resp.indexes.end(), [&index_name](const auto& i) { return i.name == index_name; });
            ...
            REQUIRE(index->keyspace_id == integration.ctx.bucket);
            REQUIRE(index->state == "online");
            REQUIRE(index->namespace_id == "default");
        }
