...
{
  mcbp_output_queue queue;
  // Asserted rather than discarded: reset() has to have something to clear, and every assertion
  // below also holds for a queue that never filled.
  assert_true(queue.enqueue(byte_buffer(std::byte{ 0x01 })),
              "the first enqueue takes the queue from idle to scheduled");
  assert_true(queue.begin_writing(), "the staged buffer is handed out as the batch in flight");
  assert_false(queue.enqueue(byte_buffer(std::byte{ 0x02 })),
               "a buffer queued behind an in-flight write does not re-arm the dispatch");
