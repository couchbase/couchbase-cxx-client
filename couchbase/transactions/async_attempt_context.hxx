 ...
 *
 * In the example below, we get 3 documents in parallel, and update each when the get returns the
 * document:
 *
...
public:
   ...
   * in subsequent calls to @ref async_attempt_context::replace or @ref
   * async_attempt_context::remove
   *
              ...
              async_result_handler&& handler)
  {
    if constexpr (std::is_same_v<Content, std::vector<std::byte>>) {
      return insert_raw(
        std::forward<std::vector<std::byte>>(content), std::move(id), content, std::move(handler));
    } else {
      // TODO: transcoder support
      return insert_raw(
        coll, std::move(id), codec::tao_json_serializer::serialize(content), std::move(handler));
    }
  }
  /**
  ...
  void replace(transaction_get_result doc, Content&& content, async_result_handler&& handler)
  {
    if constexpr (std::is_same_v<Content, std::vector<std::byte>>) {
      return replace_raw(
        std::move(doc), std::forward<std::vector<std::byte>>(content), std::move(handler));
    } else {
      // TODO: transcoder support
      return replace_raw(
        std::move(doc), codec::tao_json_serializer::serialize(content), std::move(handler));
    }
  }
  /**
   * Perform a query, within a scope.
   *
   * Performs a query given a specific scope.   Note that all subsequent transaction operations will
   * be handled by the query service.
   *
