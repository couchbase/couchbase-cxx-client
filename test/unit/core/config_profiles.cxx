...
{
  couchbase::core::cluster_options opts{};
  // The registry is a process-wide singleton and register_profile() emplaces, so a name already
  // taken is a silent no-op. Each case registers under its own name; sharing one would leave the
  // second registration doing nothing and the case asserting against the first one's profile.
  couchbase::core::known_profiles().register_profile<test_profile>("applied_profile");
  opts.apply_profile("applied_profile");
...
{
  couchbase::core::cluster_options opts{};
  couchbase::core::known_profiles().register_profile<test_profile>("overriding_profile");
  opts.apply_profile("wan_development");
  opts.apply_profile("overriding_profile");

  assert_eq(opts.connect_timeout.count(), 20'000, "an option only wan_development sets survives");
  assert_eq(
    opts.key_value_timeout.count(), 10, "an option both profiles set takes the later one's value");
}
