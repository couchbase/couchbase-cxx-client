...
TEST_CASE("transactions public async API: can do mutating query", "[transactions]")
{
    ...
    auto f = barrier->get_future();
    c.transactions()->run(
      [id, test_ctx = integration.ctx](std::shared_ptr<couchbase::transactions::async_attempt_context> ctx) {
...
TEST_CASE("transactions public async API: some query errors rollback", "[transactions]")
{
    ...
    auto f = barrier->get_future();
    c.transactions()->run(
      [id, id2, test_ctx = integration.ctx](std::shared_ptr<couchbase::transactions::async_attempt_context> ctx) {
          ctx->query(fmt::format(R"(INSERT INTO `{}` (KEY, VALUE) VALUES("{}", {}))", test_ctx.bucket, id2, async_content_json),
                     [id, ctx, &test_ctx](auto e, auto) {
                         CHECK_FALSE(e.ec());
                         ctx->query(
                           fmt::format(R"(INSERT INTO `{}` (KEY, VALUE) VALUES("{}", {}))", test_ctx.bucket, id, async_content_json),
                           [](auto, auto) {});
                     });
      },
      [barrier](auto e, auto res) {
