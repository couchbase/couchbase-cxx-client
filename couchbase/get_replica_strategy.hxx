...
struct get_replica_strategy_from_index_options {
  ...
  /**
   * Resolve an index the current topology has no replica for, or one whose copy
   * no node currently holds, to the next replica that is placed, instead of
   * failing with @ref errc::key_value::replica_index_out_of_bounds or
