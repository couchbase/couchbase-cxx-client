  ...
  const google::protobuf::RepeatedPtrField<protocol::sdk::kv::mutate_in::ContentOrMacro>& contents)
{
  ...
  for (auto content_or_macro : contents) {
    ...
    // Append for both the macro and content branches; previously the macro branch silently dropped
    // its encoded bytes because the append lived inside the else.
      ...
      if (s.content_size() != 1) {
        ...
        if (s.content(0).has_macro()) {
          ...
          throw performer_exception::unimplemented(
            "cannot use macro with array mutate-in operations");
        } else {
