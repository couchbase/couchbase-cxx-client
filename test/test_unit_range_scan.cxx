...
TEST_CASE("unit: range scan load balancer", "[unit]")
{
    ...
    {
        // Select three vbuckets, verify that they come from three different nodes and tell the balancer that in one node the stream has
        // ended
        std::set<std::int16_t> nodes{};
        for (auto i = 0; i < 3; i++) {
            auto v = balancer.select_vbucket();
        ...
        // Verify that the next selected vbucket will be for node 0 as the other two have an in-progress stream
        auto v = balancer.select_vbucket();
